...
# {
#   "awslogs": {
#       "data": "H4sIAAAAAAAAAO1UW2/bNhT+K4G...
#   }
# }
################################################################################
# Lambda handler
...
def parse_and_send(os_client, cw_logs):
  ...
  index_name = f'cwl-{current_date.year}.{current_date.month}.{current_date.day}'
  bulk_body = ''
...
def transform(md, log_event):
  ...
  ret['@id'] = log_event['id']
  ret['@timestamp'] = datetime.fromtimestamp(log_event['timestamp'] / 1000).isoformat()
